...
ax4 = axes[0, 3]
im4 = ax4.imshow(
    intermediates["reeig1"][0].detach().numpy(), cmap="RdBu_r", aspect="auto"
)
...
all_eigvals = np.concatenate(
    [eigvals_bimap1, eigvals_reeig1, eigvals_bimap2, eigvals_reeig2]
)
y_min, y_max = (
    max(all_eigvals[all_eigvals > 0].min() * 0.3, 1e-6),
    all_eigvals.max() * 2,
)

ax5 = axes[1, 0]
ax5.bar(
    range(len(eigvals_bimap1)), sorted(eigvals_bimap1, reverse=True), color="#3498db"
)
...
ax6 = axes[1, 1]
ax6.bar(
    range(len(eigvals_reeig1)), sorted(eigvals_reeig1, reverse=True), color="#e74c3c"
)
...
ax8 = axes[1, 3]
ax8.bar(
    range(len(eigvals_reeig2)), sorted(eigvals_reeig2, reverse=True), color="#9b59b6"
)
...
plt.suptitle(
    "SPDNet Pipeline: Computation Flow & ReEig Effect", fontsize=14, fontweight="bold"
)
plt.tight_layout()
plt.show()
