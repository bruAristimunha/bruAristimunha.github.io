...
from spd_learn.functional import log_euclidean_mean
...
ax1.fill(
    ...
    alpha=0.3,
    color="red",
    label=f"Euclidean (det={np.linalg.det(mean_euc_np):.2f})",
)
...
ax2.fill(
    ...
    alpha=0.3,
    color="purple",
    label=f"Log-Euclidean (det={np.linalg.det(mean_le_np):.2f})",
)
