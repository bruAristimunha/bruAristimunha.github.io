...
from spd_learn.modules import BrooksBatchNorm
...
# Apply BrooksBatchNorm
bn = BrooksBatchNorm(num_features=2, momentum=0.1, rebias=False)
bn.train()
Y = bn(X)
...
ax1.set_title("Before BrooksBatchNorm", fontweight="bold")
...
identity = Ellipse(
    (0, 0), 1, 1, facecolor="none", edgecolor="red", linewidth=2, linestyle="--"
)
...
ax2.set_title("After BrooksBatchNorm", fontweight="bold")

plt.suptitle("BrooksBatchNorm: Riemannian Centering", fontsize=13, fontweight="bold")
plt.tight_layout()
plt.show()
